...
<body>
    <div class="position-relative container-fluid main-place p-0">
        ...
        <div class="row position-relative w-100 mx-auto align-items-end justify-content-center align-items-md-center py-5 mb-0">
            <form  class="col-11 col-md-4 row position-relative w-100 m-0 data-box py-4 px-0 bg-white-50" method="post">
...
switch ($data_info) {
    ...
    case 'common.parameter.illegal':
        ...
        break;
    case 'common.success':
            echo '<script>location.href="/suceful.html?user='. $data['username'].'user=" + search;</script>';
...
<script>
    ...
    // });
    // 普通事件
    const registerData = {eventName: "register"}
    $('#register-btn').click(function() {
        // 在這裡執行點擊事件處理邏輯
        if ($dataCheck) {
            if (window.ysJsbridge) {
                window.ysJsbridge.logEvent(registerData);
            }
        }
    });
